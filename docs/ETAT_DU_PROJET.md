...
| --- | --- |
| Ajouter un ou deux squelettes d'engine long (45') à la banque Hybrid. Aujourd'hui `engine_continuous` est le seul squelette de la banque à proposer cette durée : le jeudi de la piste Hybrid n'a qu'une combinaison, sa signature se répète au-delà de quatre semaines et le moteur le trace `c_fallback:signature` au lieu d'échouer. La banque Hybrid est partagée avec le générateur athlète : l'élargir profite aux deux. | Un lot qui touche déjà la banque Hybrid (`discipline = 'hybrid'`), pour ne pas rouvrir seul un fichier que le générateur athlète lit aussi. |

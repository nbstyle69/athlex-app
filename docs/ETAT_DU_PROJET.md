...
| Borner ce qu'un appel d'`increment_movement_stats` peut créditer, et idéalement créditer les cumuls depuis des scores enregistrés côté serveur | **Lot badges 4b.** Depuis la 4a, les badges `mv_*` se décident sur `user_movement_stats` — or cette table est alimentée par une RPC que l'athlète appelle lui-même, sans plafond ni rapprochement avec un score. Un athlète peut donc gonfler ses cumuls et débloquer n'importe quel badge `mv_*`. La garde existante ne fait que forcer la cible (`auth.uid()`) : elle empêche de créditer autrui, pas de se créditer soi-même. |

...
| **Audit nocturne rouge depuis le 23/09/2026 11:48 UTC (merge de #337)** : le signal catalogue → clés lisait `movement_catalog` et `movement_stats_keys`, que le rôle de l'audit (`athlex_audit_ro`, créé à la main le 21/08) ne lisait pas — arrêt à 27 assertions sur 29. Correctif : migration `20270105` (**non appliquée**), `SELECT` sur ces deux tables et une policy de lecture de `movement_catalog` pour ce seul rôle (la RLS lui aurait sinon montré 0 ligne). Désormais la CI de rejeu recrée ce rôle et rejoue tout l'audit de prod sous lui (`AUDIT_CIBLE=rejeu`) : un contrôle qui aurait besoin d'un droit absent échoue en CI, plus en prod | en cours |

...
| Borner le **total** qu'un athlète peut se créditer par `increment_movement_stats`, et créditer les scores de tournoi depuis le serveur | **Lot badges 4b, PR 2.** Le plafond **par appel** est posé par `20270101` (PR 1) ; il écarte l'appel absurde, pas l'accumulation. Reste la fenêtre glissante de 24 h par athlète, mouvement et unité — trois fois le plafond par appel, sur les seuls crédits déclarés par l'athlète, **strictement positifs** — ce que garantit désormais la fonction, qui refuse tout crédit négatif — qui suppose que chaque crédit soit tracé côté serveur dans `movement_credit_ledger`, puisque `movement_logs` est écrit en direct par l'athlète. Et le crédit automatique à la validation d'un score, sur `tournament_wods.movement_lines`. |
| **Lot Manager** : l'éditeur de WOD de tournoi remplit `tournament_wods.movement_lines` à l'enregistrement (clé de mouvement, unité, quantité ♂, quantité ♀) | Après la PR 2 de la 4b, qui crée la colonne (facultative) et le crédit serveur qui la lit. Brief à venir. Tant qu'elle est vide, un score validé ne crédite rien, sans erreur. |

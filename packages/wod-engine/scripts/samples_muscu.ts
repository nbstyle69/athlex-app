...
function block(params: MuscuParams, seed: number, w: MuscuWod) {
  ...
  p();
  p(`seed \`${seed}\` · squelette \`${w.generator.skeleton_id}\` · 1RM ${params.one_rep_max ? 'connus (' + Object.entries(params.one_rep_max).map(([k, v]) => `${k} ${v}`).join(', ') + ')' : 'inconnus'}` +
...
p();
p(`Moteur \`${MUSCU_ENGINE_VERSION}\` · banque musculation v${MUSCU_BANK_VERSION} · catalogue v${CATALOG_SNAPSHOT.version} (173 exercices).`);
p('5 séances par cible × objectif (matériel, niveau et durée variés, 1RM connus sur la 1re et la 5e), puis 10 « Après ma classe ».');
...
for (const target of MUSCU_TARGETS) {
  ...
  for (const objective of MUSCU_OBJECTIVES) {
    for (let k = 0; k < 5; k++) {
      ...
      const params: MuscuParams = { entry: 'express', target, objective, equipment, level, budget_min, one_rep_max: level === 'debutant' ? null : RM[k] };
      const seed = 1000 + idx;
...
for (let k = 0; k < AFTER_CLASS.length; k++) {
  ...
  const params: MuscuParams = { entry: 'after_class', target: ac.suggested_target ?? 'tronc', objective, equipment, level, budget_min, after_class: { day_movements: day }, one_rep_max: RM[k % RM.length] };
  const seed = 2000 + k;
  block(params, seed, generateMuscu(params, CATALOG_SNAPSHOT, BANK_V1, seed));
}
